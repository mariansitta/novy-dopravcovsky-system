@extends('layout.auth')
@section('title', ' - ' . trans('texts.Login'))
                    ...
                    <div class="card">
                        ...
                        <div class="card-body p-4">
                            <div class="text-center mt-2">
                                ...
                                <p class="text-muted">{{ trans('texts.Login') }}</p>
                            </div>
                            <div class="p-2 mt-4">
                                <form method="POST" action="{{ route('login') }}" method="post" id="login-form">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="password" class="form-control {{ $errors->has("password") ? 'parsley-error' : '' }} login"
                                               value="{{ old('email') ? '' : request('password') }}" name="password" id="password" placeholder="{{ trans('texts.Password') }}">
                                        @include('web._partials._errors', ['column' => "password"])
                                    </div>

                                    <div class="row">
                                        <div class="col-lg-12 text-center">
                                            <button class="btn btn-primary w-sm waves-effect waves-light" type="submit" id="login-button">{{ trans('texts.Log in') }}</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
